...
class Runnable(Controllable):
    ...
    def getprocess(self) -> typing.Optional[psutil.Process]:
        ...
        ps = next((proc for proc in psutil.process_iter(['pid', 'name'],ad_value=None) if self._processfilter(proc)),None)
        return ps
...
available_functions = {'list':list_all, 'status':status, 'check': check, 'start': start, 'stop': stop, 'restart': restart}
...
def build_parser(controllable_cls: typing.Type[Controllable]) -> argparse.ArgumentParser:
    ...
    parser.add_argument('controllables',nargs='*',action='append',choices=[[]]+list(Controllable.Instances.keys()),default=[]) # type: ignore
    parser.add_argument('-a','--all', dest='doall', action='store_true',help='Apply to all registered controllables, excluding groups')
    parser.add_argument('-e','--env', type = str, default=None,help='config file environment part, e.g. test or dev (start only)')
...
def pcp():
    ...
    try:
        ...
        if args.doall or args.command == 'list':
            conts = [cont for cont in Controllable.Instances.values() if not isinstance(cont,ProcessGroup)]
        else:
