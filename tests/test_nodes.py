...
def test_nodes():
    ...
    assert next(keydirectory.NodeRegistry.get_next_proxy(ddhkey,nodes.NodeSupports.data))[0] == node_d.get_proxy()
    assert keydirectory.NodeRegistry.get_node(ddhkey_s,nodes.NodeSupports.schema,transaction)[0].schemas.current_schema is schema 
    return
...
def test_schema_node():
    ...
    node_s,split = keydirectory.NodeRegistry.get_node(ddhkey,nodes.NodeSupports.schema,transaction)
