...
def test_schema_multiple_versions():
    ...
    s,k =  schemas.SchemaContainer.get_schema_key(keys.DDHkey(key='//p/health'),transaction) # alt must be explicit
    assert s is schema_r3 # so it's still preferred
    assert str(k) == '//p/health::rec:3'

    s,k =  schemas.SchemaContainer.get_schema_key(keys.DDHkey(key='//p/health::alt'),transaction) # explicit alt, latest version
    assert s is schema_a4
    assert str(k) == '//p/health::alt:4'

    s,k =  schemas.SchemaContainer.get_schema_key(keys.DDHkey(key='//p/health::alt:2'),transaction) # explicit alt, earlier version
