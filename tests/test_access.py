...
def test_basic_access():
    ...
    node_c = DummyNode(consents=permissions.Consents(
        consents=[permissions.Consent(grantedTo=[user2])]), owner=user1)
    ...
    access = permissions.Access(ddhkey=ddhkey, principal=user2,
                                modes=[permissions.AccessMode.write])
...
@pytest.fixture
def users7():
    users7 = [users.User(id=str(id), name='user'+str(id), email='user' +
                         str(id)+'@dummy.com') for id in range(7)]
    return users7
    ...
    node_c_m = DummyMultiOwnerNode(all_owners=tuple(users7[0:2]),
                                   consents=permissions.MultiOwnerConsents(consents_by_owner={
                                       users7[0]: permissions.Consents(consents=[
                                           permissions.Consent(grantedTo=[users7[1]]),
                                           permissions.Consent(
                                               grantedTo=[users7[2]], withModes={AM.read}),
                                           permissions.Consent(
...
def test_access_modes(ddhkey_setup, users7, ok, obj, user, modes, comment):
    ...
    node, dummy = keydirectory.NodeRegistry.get_node(
        ddhkey, support=nodes.NodeSupports.data, transaction=None)
