...
def test_variant():
    ...
    # with pytest.raises(ValueError):
    #     keys.DDHkey(key='norooted/subkey:schema:supported')  # TODO: supported is not acceptable, as not unique


def test_version():
    ddhkey1 = keys.DDHkey(key='norooted:data:recommended:unspecified')
    ddhkey2 = keys.DDHkey(key='norooted/subkey:data:recommended:unspecified')
    assert ddhkey1 == ddhkey2.up()

    ddhkey1a = keys.DDHkey(key='norooted:data::unspecified')
    ddhkey1b = keys.DDHkey(key='norooted::recommended:unspecified')
    ...
    ddhkey4 = keys.DDHkey(key='norooted/subkey:data:recommended:5.0')
    assert ddhkey3 != ddhkey4
